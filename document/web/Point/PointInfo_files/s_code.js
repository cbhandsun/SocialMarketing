...
var s_account="vipshopvipshopjf"
...
s.linkInternalFilters="javascript:,vipshop.com"
...
function s_doPlugins(s) {
	...
	s.eVar12=t_day
	 var date = new Date();
	s.eVar32 = date.getFullYear().toString()+"-"+date.getMonth().toString()+"-"+date.getDate().toString(); //日期
...
+".s_c_il)window.s_c_il[pars[0]].mrq(pars[1]+'')};setTimeout(setS_c_il,750,[s._in, un])}else{l=s.rl[un];if(l){r.t=ta;r.u=un;r.r=rs;l[l.length]=r;return ''}imn+='_'+s.rc[un];s.rc[un]++}im=s.wd[imn];if(!im)im=s.wd[imn]=new Image;im."
...
/*---------------------------------------积分商城监控代码 -------------------------------------------------*/
//积分商城首页
function oit_jf_index(pageName){
s.pageName=pageName;             //传入积分商城首页
s.channel="积分商城";            //所属频道名称，其他频道名称如“闪购”，“旅行”等
s.prop4="积分商城首页";              //页面类型，所有频道首页都定义为“频道首页”
s.prop10="30s";                  //注：需要技术同事将页面加载时间赋值给此变量
}
//积分商城礼品列表页
function oit_jf_list(pageName){
s.pageName=pageName;             //传入积分礼品列表页或唯品礼券列表页或积分活动列表页
s.channel="积分商城";            //所属频道名称，其他频道名称如“闪购”，“旅行”等
s.prop4="礼品列表页";              //页面类型，所有频道首页都定义为“频道首页”
s.prop10="30s";                  //注：需要技术同事将页面加载时间赋值给此变量
}
//积分商城礼品详情页
function oit_jf_detail(pageName,products){
s.pageName=pageName;                                //页面名称
s.channel="积分商城";                                 //所属频道名称
s.prop4="礼品详情页";  
s.products=";"+products;  //产品信息（格式为s.products=";商品ID"）
s.eVar16=pageName;
s.events="prodView,event3" ;       
s.prop10="30s";                                        //注：需要技术同事将页面加载时间赋值给此变量
}
//积分兑换事件
function oit_jf_cart(oit_products_id,num,total)
{                                                             
s.linkTrackVars='products,eVar10,events';
s.linkTrackEvents='purchase';       
s.events='purchase';                                                                    
s.products=';'+oit_products_id+";"+num+";"+total;  //注：仅添加至购物车的产品方可列入products变量。  
s.tl(this,'o','exchange success');   
}
//公告页面
function oit_jf_rul(pageName){
s.pageName=pageName;             
s.channel="积分商城";            //所属频道名称，其他频道名称如“闪购”，“旅行”等
s.prop4="公告页面";              //页面类型，所有频道首页都定义为“频道首页”
s.prop10="30s";                  //注：需要技术同事将页面加载时间赋值给此变量	
}